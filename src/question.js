...
const Question = () => {
  ...
  const Button1 = () =>{
    resetTimer();
    const Option  = "A"
    axios.post("https://13.51.85.151/answer/check/question/correct/or/wrong",{Option, Qno, ip})
    .then(res =>{
      if(res.data.Status === "OK"){
        axios.delete(`https://13.51.85.151/delete/data/api/dont/know/ada/upi/${upii1._id}`)
        .then(res =>{
          if(res.data.Status === "OK"){
            axios.delete(`https://13.51.85.151/delete/data/api/dont/know/ada/upi/one/exts/${upii2._id}`)
            .then(res =>{
              if(res.data.Status === "OK"){
