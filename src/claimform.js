...
const Claimform = () => {
    ...
    useEffect(()=>{
        ...
        fetch(`https://16.170.242.41/question/singel/verify/data/01/${ip}`)
        .then(res => res.json())
    ...
    const Post = (e) =>{
        ...
        axios.post("https://16.170.242.41/account/data/post/get",{ip, Country, upi,name})
        .then(res =>{
            if(res.data.Status === "OK"){
                axios.delete(`https://16.170.242.41/delete/data/api/dont/know/${veri._id}`)
                .then(res =>{
                    if(res.data.Status === "OK"){
